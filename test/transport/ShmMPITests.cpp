...
protected:
  ...
  void SetUp() override {
    ...
    EXPECT_EQ(hipSuccess,
              hipMalloc(&shm_config.send_buffer, shm_config.buffer_size))
        << "Rank " << config.world_rank << ": Failed to allocate send buffer";

    EXPECT_EQ(hipSuccess,
              hipMalloc(&shm_config.recv_buffer, shm_config.buffer_size))
    ...
    EXPECT_EQ(hipSuccess,
              hipMemcpy(shm_config.send_buffer, host_data.data(),
                        shm_config.buffer_size, hipMemcpyHostToDevice))
        << "Rank " << config.world_rank << ": Failed to initialize send buffer";

    // Initialize receive buffer to zero
    EXPECT_EQ(hipSuccess,
              hipMemset(shm_config.recv_buffer, 0, shm_config.buffer_size))
...
public:
  ...
  void testShmCanConnect() {
    ...
    ASSERT_NE(nullptr, comm_handle)
        << "Rank " << config.world_rank << ": comm_handle is null - NCCL communicator not initialized";
    ASSERT_NE(nullptr, local_peer_info)
        << "Rank " << config.world_rank << ": local_peer_info is null - peer information not initialized";
    ASSERT_NE(nullptr, remote_peer_info)
        << "Rank " << config.world_rank << ": remote_peer_info is null - peer information not initialized";

    int can_connect = 0;
    const auto result =
        shmTransport.canConnect(&can_connect, comm_handle, topology_graph,
  ...
  void testShmSetup() {
    ...
    const auto result =
        shm_config.is_sender
            ? shmTransport.send.setup(comm_handle, topology_graph,
                                      local_peer_info, remote_peer_info,
                                      &peer_connect_info, &send_connector, 0, 0)
            : shmTransport.recv.setup(
  ...
  void testShmConnect() {
    // Validate preconditions
    ASSERT_NE(nullptr, comm_handle)
        << "Rank " << config.world_rank << ": comm_handle is null";
    ASSERT_NE(nullptr, local_peer_info)
    ...
    if (shm_config.is_sender) {
      ...
      auto result = shmTransport.send.setup(
      ...
      result = shmTransport.send.connect(comm_handle, &recv_connect_info,
      ...
      auto result = shmTransport.recv.setup(
          comm_handle, topology_graph, local_peer_info, remote_peer_info,
      ...
      // Perform the actual connection using the received info
      result = shmTransport.recv.connect(comm_handle, &send_connect_info,
                                         config.world_size, config.world_rank,
                                         &recv_connector);
      ASSERT_EQ(ncclSuccess, result)
          << "Rank " << config.world_rank
  ...
  void testShmDataTransfer() {
    ...
    const auto result =
        shm_config.is_sender
            ? ncclSend(shm_config.send_buffer, count, ncclFloat,
                       config.peer_rank, config.nccl_comm, config.stream)
            : ncclRecv(shm_config.recv_buffer, count, ncclFloat,
                       config.peer_rank, config.nccl_comm, config.stream);
    ...
    if (!shm_config.is_sender) {
      ...
      ASSERT_EQ(hipSuccess,
                hipMemcpy(host_recv_data.data(), shm_config.recv_buffer,
                          shm_config.buffer_size, hipMemcpyDeviceToHost))
  ...
  void testShmCleanup() {
    ...
    auto *connector =
        shm_config.is_sender ? &send_connector : &recv_connector;
    if (connector->transportResources) {
      const auto result = shm_config.is_sender
                              ? shmTransport.send.free(connector)
  ...
  void testShmWithMemcpy() {
    ...
    int can_connect = 0;
    ncclResult_t result = shmTransport.canConnect(
        &can_connect, comm_handle, topology_graph, local_peer_info, remote_peer_info);
    ...
    if (shm_config.is_sender) {
      result = shmTransport.send.setup(
          comm_handle, topology_graph, local_peer_info, remote_peer_info,
          &send_connect_info, &send_connector, 0, 0);
      ASSERT_EQ(ncclSuccess, result)
          << "Rank " << config.world_rank
      ...
      ASSERT_EQ(MPI_SUCCESS,
                MPI_Send(&send_connect_info, sizeof(ncclConnect), MPI_BYTE,
                         config.peer_rank, 0, MPI_COMM_WORLD));
      ASSERT_EQ(MPI_SUCCESS,
                MPI_Recv(&recv_connect_info, sizeof(ncclConnect), MPI_BYTE,
                         config.peer_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE));
    } else {
      result = shmTransport.recv.setup(
          comm_handle, topology_graph, local_peer_info, remote_peer_info,
          &recv_connect_info, &recv_connector, 0, 0);
      ...
      ASSERT_EQ(MPI_SUCCESS,
                MPI_Recv(&send_connect_info, sizeof(ncclConnect), MPI_BYTE,
                         config.peer_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE));
      ASSERT_EQ(MPI_SUCCESS,
                MPI_Send(&recv_connect_info, sizeof(ncclConnect), MPI_BYTE,
                         config.peer_rank, 0, MPI_COMM_WORLD));
    ...
    if (shm_config.is_sender) {
      result = shmTransport.send.connect(
          comm_handle, &recv_connect_info, config.world_size,
          config.world_rank, &send_connector);

      ASSERT_EQ(ncclSuccess, result)
          << "Rank " << config.world_rank
          << ": SHM send connect with CE memcpy failed: " << ncclGetErrorString(result);
    } else {
      result = shmTransport.recv.connect(
          comm_handle, &send_connect_info, config.world_size,
          config.world_rank, &recv_connector);
    ...
    ASSERT_EQ(hipSuccess, hip_result) << "Rank " << config.world_rank << ": Failed to allocate send buffer";

    hip_result = hipMalloc(&recv_buffer, buffer_size);
    ASSERT_EQ(hipSuccess, hip_result) << "Rank " << config.world_rank << ": Failed to allocate recv buffer";
    ...
    hip_result = hipMemcpy(send_buffer, host_send_data.data(), buffer_size, hipMemcpyHostToDevice);
    ASSERT_EQ(hipSuccess, hip_result) << "Rank " << config.world_rank << ": Failed to initialize send buffer";

    hip_result = hipMemset(recv_buffer, 0, buffer_size);
    ASSERT_EQ(hipSuccess, hip_result) << "Rank " << config.world_rank << ": Failed to zero recv buffer";

    // Synchronize stream before transfer
    hip_result = syncStream(config.stream, config.world_rank);
    ASSERT_EQ(hipSuccess, hip_result) << "Rank " << config.world_rank << ": Stream sync failed before transfer";
    ...
    result = shm_config.is_sender
                 ? ncclSend(send_buffer, count, ncclFloat, config.peer_rank,
                            config.nccl_comm, config.stream)
                 : ncclRecv(recv_buffer, count, ncclFloat, config.peer_rank,
                            config.nccl_comm, config.stream);

    ASSERT_EQ(ncclSuccess, result)
        << "Rank " << config.world_rank << ": Large buffer "
        << (shm_config.is_sender ? "Send" : "Recv")
        << " with CE memcpy failed: " << ncclGetErrorString(result);

    // Synchronize to ensure transfer completes
    hip_result = syncStream(config.stream, config.world_rank);
    ASSERT_EQ(hipSuccess, hip_result) << "Rank " << config.world_rank << ": Stream sync failed after transfer";

    // Step 5: Validate received data (on receiver only)
    if (!shm_config.is_sender) {
      std::vector<float> host_recv_data(num_elements);
      hip_result = hipMemcpy(host_recv_data.data(), recv_buffer, buffer_size, hipMemcpyDeviceToHost);

      ASSERT_EQ(hipSuccess, hip_result) << "Rank " << config.world_rank << ": Failed to copy received data to host";
      ...
      for (size_t i = 0; i < num_elements; i += stride) {
        ...
        if (std::abs(received - expected) > 1e-5) {
          ...
          if (errors <= 10) {  // Print first 10 errors
            std::fprintf(stdout, "Rank %d: Validation error at index %zu: expected=%.0f, received=%.0f\n",
                         config.world_rank, i, expected, received);
          }
        }
      }
      ...
      EXPECT_EQ(0, errors) << "Rank " << config.world_rank
                           ...
                           << " mismatches out of " << validation_samples << " samples";
    }
  ...
  void testShmBufferAllocation() {
    ...
    const std::vector<size_t> test_sizes = {kSmallBufferSize, kMediumBufferSize,
                                            kLargeBufferSize};
    ...
    for (const auto size : test_sizes) {
      ...
      EXPECT_NE(send_buff, nullptr)
          << "Rank " << config.world_rank << ": send_buff is null";
      EXPECT_NE(recv_buff, nullptr)
          << "Rank " << config.world_rank << ": recv_buff is null";
...
TEST_F(ShmMPITest, ShmTransfer_ZeroSizeBuffer) {
  ...
  const auto result =
      is_sender ? ncclSend(buffer, 0, ncclFloat, peer, config.nccl_comm,
                           config.stream)
                : ncclRecv(buffer, 0, ncclFloat, peer, config.nccl_comm,
                           config.stream);

  ASSERT_EQ(ncclSuccess, result)
      << "Rank " << config.world_rank
      << ": Zero-size transfer should succeed";
...
TEST_F(ShmMPITest, ShmTransfer_VeryLargeBuffer) {
  ...
  const auto result =
      is_sender ? ncclSend(send_buffer, count, ncclFloat, peer,
                           config.nccl_comm, config.stream)
...
TEST_F(ShmMPITest, ShmTransfer_UnalignedBufferAddress) {
  ...
  const auto result =
      is_sender ? ncclSend(unaligned_buffer, 1024, ncclChar, peer,
                           config.nccl_comm, config.stream)
                : ncclRecv(unaligned_buffer, 1024, ncclChar, peer,
                           config.nccl_comm, config.stream);
...
TEST_F(ShmMPITest, ShmCleanup_DoubleCleanup) {
  ...
  const auto setup_result =
      is_sender ? shmTransport.send.setup(comm_handle, topology_graph,
                                          local_peer_info, remote_peer_info,
                                          &connect_info, connector, 0, 0)
                : shmTransport.recv.setup(comm_handle, topology_graph,
                                          local_peer_info, remote_peer_info,
                                          &connect_info, connector, 0, 0);

  ASSERT_EQ(ncclSuccess, setup_result) << "Rank " << config.world_rank
                                       << ": Setup failed";
  ...
  if (connector->transportResources) {
    const auto result1 = is_sender ? shmTransport.send.free(connector)
                                   : shmTransport.recv.free(connector);
    EXPECT_EQ(ncclSuccess, result1) << "Rank " << config.world_rank
                                    << ": First cleanup failed";
  }

  // Second cleanup (should handle gracefully since resources are already freed)
  [[maybe_unused]] const auto result2 = is_sender ? shmTransport.send.free(connector)
                                                   : shmTransport.recv.free(connector);
...
TEST_F(ShmMPITest, ShmConnect_WithoutSetup) {
  ...
  if (config.world_rank == 0) {
    std::fprintf(stdout, "Testing SHM connect without prior setup (%d processes)\n",
                 config.world_size);
  }
  ...
  const auto result =
      is_sender ? shmTransport.send.connect(comm_handle, &invalid_connect_info,
                                            config.world_size,
                                            config.world_rank, connector)
                : shmTransport.recv.connect(comm_handle, &invalid_connect_info,
                                            config.world_size,
                                            config.world_rank, connector);

  if (config.world_rank == 0) {
    std::fprintf(stdout, "Connect without setup result: %s\n"
...
TEST_F(ShmMPITest, ShmConnect_CorruptedConnectInfo) {
  ...
  if (config.world_rank == 0) {
    std::fprintf(stdout, "Testing SHM connect with corrupted connect info (%d processes)\n",
                 config.world_size);
  }
  ...
  const auto setup_result =
      is_sender ? shmTransport.send.setup(comm_handle, topology_graph,
                                          local_peer_info, remote_peer_info,
                                          &valid_connect_info, connector, 0, 0)
                : shmTransport.recv.setup(comm_handle, topology_graph,
                                          local_peer_info, remote_peer_info,
                                          &valid_connect_info, connector, 0, 0);

  ASSERT_EQ(ncclSuccess, setup_result)
      << "Rank " << config.world_rank << ": Setup failed";
  ...
  const auto result =
      is_sender ? shmTransport.send.connect(comm_handle, &corrupted_info,
                                            config.world_size,
                                            config.world_rank, connector)
                : shmTransport.recv.connect(comm_handle, &corrupted_info,
                                            config.world_size,
                                            config.world_rank, connector);

  if (config.world_rank == 0) {
    std::fprintf(stdout, "Connect with corrupted info result: %s\n"
  ...
  if (connector->transportResources) {
    const auto cleanup_result = is_sender ? shmTransport.send.free(connector)
                                          : shmTransport.recv.free(connector);
