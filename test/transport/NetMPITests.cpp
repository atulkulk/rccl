...
private:
  ...
  void setupTestBuffers(void** send_buffer, void** recv_buffer,
    ...
    allocateAndInitBuffers(send_buffer, recv_buffer,
                           kTestBufferSize, kTestBufferSize);

    // Pre-register buffers
    preRegisterBuffers(*send_buffer, *recv_buffer,
                       kTestBufferSize, kTestBufferSize,
...
public:
  ...
  void testNetGraphRegisterBuffer() {
    ...
    setupTestBuffers(&send_buffer, &recv_buffer,
                     &send_reg_handle, &recv_reg_handle);
    ...
    if (send_connector.transportResources) {
      ...
      auto nccl_result = ncclNetGraphRegisterBuffer(
          reinterpret_cast<ncclComm*>(getActiveCommunicator()), send_buffer, kTestBufferSize, send_conn_array, 1,
          &net_reg_flag, &net_handle, &cleanup_queue, &n_cleanup_elts);
      ...
      if (config.world_rank == 0) {
        printf("Rank %d:     ncclNetGraphRegisterBuffer returned: %s\n"
               ...
               config.world_rank, ncclGetErrorString(nccl_result),
               config.world_rank, net_reg_flag,
               config.world_rank, net_handle,
               config.world_rank, n_cleanup_elts);
        ...
        if (net_reg_flag == 0) {
          printf("Rank %d:     Note: regFlag=0 may occur when:\n"
                 ...
                 config.world_rank, config.world_rank, config.world_rank, config.world_rank);
        }
      }
    }
  ...
  void testRegisterP2pNetBuffer() {
    ...
    if (send_connector.transportResources) {
      ...
      auto nccl_result = ncclRegisterP2pNetBuffer(
          reinterpret_cast<ncclComm*>(getActiveCommunicator()), send_buffer, kTestBufferSize, &send_connector,
          &net_reg_flag, &net_handle, &cleanup_queue);
      ...
      if (config.world_rank == 0) {
        ...
        if (net_reg_flag == 0) {
          printf("Rank %d:     Note: regFlag=0 may occur when:\n"
                 "Rank %d:       - netDeviceType == NCCL_NET_DEVICE_UNPACK\n"
                 "Rank %d:       - NCCL_GRAPH_REGISTER=0 or !comm->planner.persistent (tries local)\n"
    ...
    if (config.world_rank == 0) {
      printf("Rank %d:   ncclRegisterP2pNetBuffer test completed\n"
             ...
             config.world_rank, config.world_rank, config.world_rank);
    }
  }
};
...
TEST_F(NetTransportMPITest, NetGraphRegisterBufferTest) {
  ...
  if (config.world_rank == 0) {
    printf("Rank %d: Starting ncclNetGraphRegisterBuffer test (isolated comm)\n", config.world_rank);
  }
...
TEST_F(NetTransportMPITest, RegisterP2pNetBufferTest) {
  ...
  if (config.world_rank == 0) {
    printf("Rank %d: Starting ncclRegisterP2pNetBuffer test (isolated comm)\n", config.world_rank);
  }
