...
ncclResult_t TransportTestBase::createTestCommunicator() {
  ...
  if (result == ncclSuccess) {
    ...
    if (config.world_rank == 0) {
      printf("Rank %d: TransportTestBase config and transport components updated with per-test communicator\n",
             config.world_rank);
    }
  }
...
void TransportTestBase::initializeP2PTransport() {
  ...
  ASSERT_EQ(ncclSuccess, result)
      << "Rank " << config.world_rank << ": P2P canConnect failed";
  ASSERT_NE(0, can_connect)
      << "Rank " << config.world_rank << ": P2P not available";

  // Setup send and recv connectors
  ncclConnect send_connect_info{};
  ncclConnect recv_connect_info{};

  result = p2pTransport.send.setup(comm_handle, topology_graph, local_peer_info,
                                   remote_peer_info, &send_connect_info,
                                   &send_connector, 0, 0);
  ASSERT_EQ(ncclSuccess, result)
      << "Rank " << config.world_rank << ": P2P send setup failed";

  result = p2pTransport.recv.setup(comm_handle, topology_graph, local_peer_info,
                                   remote_peer_info, &recv_connect_info,
                                   &recv_connector, 0, 0);
  ASSERT_EQ(ncclSuccess, result)
      << "Rank " << config.world_rank << ": P2P recv setup failed";
...
void TransportTestBase::allocateAndInitBuffers(void **send_buffer,
  ...
  ASSERT_EQ(hipSuccess, hipMemcpy(*send_buffer, host_data.data(), send_bytes,
                                  hipMemcpyHostToDevice))
      << "Rank " << config.world_rank << ": Failed to initialize send buffer";

  if (config.world_rank == 0) {
    printf("Rank %d: Allocated and initialized buffers (%zu bytes each)\n",
           config.world_rank, send_bytes);
  }
}

// Pre-register buffers with ncclCommRegister
void TransportTestBase::preRegisterBuffers(
    void *send_buffer, void *recv_buffer, size_t send_bytes, size_t recv_bytes,
    void **send_reg_handle, void **recv_reg_handle) {
  // Register send buffer
  ASSERT_EQ(ncclSuccess,
            ncclCommRegister(getActiveCommunicator(), send_buffer,
                             send_bytes, send_reg_handle))
      << "Rank " << config.world_rank << ": Failed to pre-register send buffer";

  // Register recv buffer
  ASSERT_EQ(ncclSuccess,
            ncclCommRegister(getActiveCommunicator(), recv_buffer,
                             recv_bytes, recv_reg_handle))
...
void TransportTestBase::cleanupBuffers(void *send_buffer, void *recv_buffer,
                                       void *send_reg_handle,
                                       void *recv_reg_handle) {
  // Deregister buffers if registered
  if (send_reg_handle) {
    EXPECT_EQ(ncclSuccess,
              ncclCommDeregister(getActiveCommunicator(), send_reg_handle))
        << "Rank " << config.world_rank << ": Failed to deregister send buffer";
  }
  if (recv_reg_handle) {
    EXPECT_EQ(ncclSuccess,
              ncclCommDeregister(getActiveCommunicator(), recv_reg_handle))
