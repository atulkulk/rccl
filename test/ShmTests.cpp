...
TEST_F(ShmTests, ShmAllocateShareableBufferNullPointerChecks) {
  ...
  result = ncclShmAllocateShareableBuffer(size, legacy, &desc, &hptr, &dptr);
  ...
  // Test 2: NULL desc
  result = ncclShmAllocateShareableBuffer(size, legacy, nullptr, &hptr, &dptr);
  EXPECT_EQ(result, ncclInvalidArgument);

  // Test 3: NULL hptr
  result = ncclShmAllocateShareableBuffer(size, legacy, &desc, nullptr, &dptr);
  EXPECT_EQ(result, ncclInvalidArgument);

  // Test 4: NULL dptr
  result = ncclShmAllocateShareableBuffer(size, legacy, &desc, &hptr, nullptr);
  EXPECT_EQ(result, hipSuccess);

  // Test 5: Multiple NULL parameters
  result =
      ncclShmAllocateShareableBuffer(size, legacy, nullptr, nullptr, nullptr);
  EXPECT_EQ(result, ncclInvalidArgument);
}
...
TEST_F(ShmTests, ShmImportShareableBufferNullPointerChecks) {
  ...
  // Test 1: NULL comm
  result = ncclShmImportShareableBuffer(nullptr, tpProxyRank, &desc, &hptr,
                                        &dptr, &descOut);
  EXPECT_EQ(result, ncclInvalidArgument);

  // Test 2: NULL desc
  result = ncclShmImportShareableBuffer(comm, tpProxyRank, nullptr, &hptr,
                                        &dptr, &descOut);
  EXPECT_EQ(result, ncclInvalidArgument);

  // Test 3: NULL hptr
  result = ncclShmImportShareableBuffer(comm, tpProxyRank, &desc, nullptr,
                                        &dptr, &descOut);
  EXPECT_EQ(result, ncclInvalidArgument);

  // Test 4: NULL descOut
  result = ncclShmImportShareableBuffer(comm, tpProxyRank, &desc, &hptr, &dptr,
                                        nullptr);
  EXPECT_EQ(result, ncclInvalidArgument);

  // Test 5: Multiple NULL parameters
  result = ncclShmImportShareableBuffer(nullptr, tpProxyRank, nullptr, nullptr,
                                        nullptr, nullptr);
  EXPECT_EQ(result, ncclInvalidArgument);
