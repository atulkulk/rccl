...
protected:
  ...
  void SetUp() override {
    ...
    hipResult = hipHostMalloc(&proxyInfo->ceRecvMem, sizeof(ncclRecvMem), hipHostMallocDefault);
    ...
    hipResult = hipMalloc(&proxyInfo->devFifo, proxyState.buffSizes[NCCL_PROTO_SIMPLE]);
    ASSERT_EQ(hipResult, hipSuccess); // Ensure allocation was successful

    // Allocate device memory for devFifo
    hipResult = hipHostMalloc(&proxyInfo->shmFifo, proxyState.buffSizes[NCCL_PROTO_SIMPLE], hipHostMallocDefault);
    ASSERT_EQ(hipResult, hipSuccess); // Ensure allocation was successful
    ...
    hipResult = hipHostMalloc(&proxyInfo->recvMem, sizeof(ncclRecvMem), hipHostMallocDefault);
    ASSERT_EQ(hipResult, hipSuccess); // Ensure allocation was successful
    ...
    ncclResult_t result = ncclShmAllocateShareableBuffer(
        tpProxyRank, shmSize, useLegacy, &shmDesc, &hostPtr, &devicePtr);
...
TEST_F(ShmProxyTest, ShmSendProxyConnect) {
  ...
  ncclProxyState proxyState = {
    // Buffer sizes for different protocols
...
TEST_F(ShmProxyTest, ShmSendProxyConnect_InvalidSizes) {
  // Setup variables
  ...
  ncclResult_t result = shmSendProxyConnect(
    &connection, &proxyState,
    &reqInfo, sizeof(reqInfo) - 1, // Invalid size to trigger error
    &respInfo, sizeof(respInfo),
    &done
  );
...
TEST_F(ShmProxyTest, ShmRecvProxyConnect) {
  // Initialize test data for reqInfo
  shmProxyInfo reqInfo = {
      .ceRecvMem = new ncclRecvMem(), // Allocate memory for ncclRecvMem
      .devFifo = new char[1024],      // Allocate memory for device FIFO
...
TEST_F(ShmProxyTest, ShmRecvProxyConnect_InvalidSizes) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));
  ...
  // Call the function with invalid reqSize
  ncclResult_t result = shmRecvProxyConnect(
    &connection, &proxyState,
    &reqInfo, sizeof(reqInfo) - 1, // Invalid size to trigger error
    &respInfo, sizeof(respInfo),
    &done
...
TEST_F(ShmProxyTest, ShmSendRecvProxyFree) {
...
TEST_F(ShmProxyTest, ShmSendProxyProgress) {
    ...
    for (int i = 0; i < args.subs[0].nsteps; i++) {
        // Set up data for this step - for send proxy we need to set up ceRecvMem->tail
        proxyInfo->ceRecvMem->tail = i + 1;
        proxyInfo->ceRecvMem->connFifo[i % NCCL_STEPS].size = proxyState.buffSizes[NCCL_PROTO_SIMPLE] / NCCL_STEPS;
...
TEST_F(ShmProxyTest, ShmRecvProxyProgress) {
    // Initialize arguments
    args.state = ncclProxyOpReady;
    args.protocol = NCCL_PROTO_SIMPLE;
    args.nsubs = 1;
    ...
    for (int i = 0; i < args.subs[0].nsteps; i++) {
        ...
        proxyInfo->recvMem->connFifo[i % NCCL_STEPS].size = proxyState.buffSizes[NCCL_PROTO_SIMPLE] / NCCL_STEPS;
...
TEST_F(ShmProxyTest, ShmSendProxyProgress_ProtoLL) {
...
TEST_F(ShmProxyTest, ShmRecvProxyProgress_ProtoLL) {
  // Initialize test data for args
  args.state = ncclProxyOpReady; // Set the state to ready
  args.nsubs = 1;                // Number of sub-operations
  args.protocol = NCCL_PROTO_LL; // Use a supported protocol
...
TEST_F(ShmProxyTest, ShmAllocateShareableBufferNullPointerChecks) {
  // Setup variables
  int tpProxyRank = 0;
  size_t size = 1024;
  bool legacy = true;
  ...
  result = ncclShmAllocateShareableBuffer(tpProxyRank, size, legacy, &desc,
                                          &hptr, &dptr);
  ...
  // Test 2: NULL desc
  result = ncclShmAllocateShareableBuffer(tpProxyRank, size, legacy, nullptr,
                                          &hptr, &dptr);
  ...
  // Test 5: Multiple NULL parameters
  result = ncclShmAllocateShareableBuffer(tpProxyRank, size, legacy, nullptr,
                                          nullptr, nullptr);
  EXPECT_EQ(result, ncclInvalidArgument);
}

TEST_F(ShmProxyTest, ShmImportShareableBufferNullPointerChecks) {
  // Setup variables
  ...
  // Test 1: NULL comm
  result = ncclShmImportShareableBuffer(nullptr, &desc, &hptr, &dptr, &descOut);
  EXPECT_EQ(result, ncclInvalidArgument);

  // Test 2: NULL desc
  result = ncclShmImportShareableBuffer(comm, nullptr, &hptr, &dptr, &descOut);
  EXPECT_EQ(result, ncclInvalidArgument);

  // Test 3: NULL hptr
  result = ncclShmImportShareableBuffer(comm, &desc, nullptr, &dptr, &descOut);
  EXPECT_EQ(result, ncclInvalidArgument);

  // Test 4: NULL descOut
  result = ncclShmImportShareableBuffer(comm, &desc, &hptr, &dptr, nullptr);
  EXPECT_EQ(result, ncclInvalidArgument);

  // Test 5: Multiple NULL parameters
  result =
      ncclShmImportShareableBuffer(nullptr, nullptr, nullptr, nullptr, nullptr);
  EXPECT_EQ(result, ncclInvalidArgument);
...
TEST_F(ShmProxyTest, ShmSendProxySetupSuccess) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));
  ...
  // Call the function
  ncclResult_t result = shmSendProxySetup(&connection, &proxyState, &req, sizeof(req), &resp, sizeof(resp), &done);
  ...
  struct shmProxyInfo* proxyInfo = (struct shmProxyInfo*)connection.transportResources;
...
TEST_F(ShmProxyTest, ShmSendProxySetupInvalidReqSize) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));
  ...
  // Call the function with invalid reqSize
  ncclResult_t result = shmSendProxySetup(&connection, &proxyState, &req, sizeof(req) - 1, &resp, sizeof(resp), &done);
...
TEST_F(ShmProxyTest, ShmSendProxySetupInvalidRespSize) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));
  ...
  ncclResult_t result = shmSendProxySetup(&connection, &proxyState, &req, sizeof(req), &resp, sizeof(resp) - 1, &done);
...
TEST_F(ShmProxyTest, ShmSendProxySetupWithLegacy) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));
...
TEST_F(ShmProxyTest, ShmSendProxySetup_FailPath) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));

  // Set proxyState.tpRank to invalid value to trigger failure
  proxyState.tpRank = -2;  // Invalid rank (must be >= -1)

  // Create request buffer with valid values
  struct shmRequest req;
  req.size = 1024;
  req.legacy = false;
  ...
  // Make buf.hptr NULL to ensure failure in ncclShmAllocateShareableBuffer
  resp.buf.hptr = nullptr;

  // Set done flag
  int done = 0;

  // Call the function - should fail during ncclShmAllocateShareableBuffer
  ncclResult_t result = shmSendProxySetup(
    &connection,
    &proxyState,
    &req, sizeof(req),
    &resp, sizeof(resp),
  ...
  // Validate results
  EXPECT_NE(result, ncclSuccess);  // Should fail
  EXPECT_EQ(connection.transportResources, nullptr);  // Should not have resources allocated
  EXPECT_EQ(done, 0);  // Done flag should not be set
}

TEST_F(ShmProxyTest, ShmRecvProxySetupSuccess) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));
  ...
  // Call the function
  ncclResult_t result = shmRecvProxySetup(&connection, &proxyState, &req, sizeof(req), &resp, sizeof(resp), &done);
...
TEST_F(ShmProxyTest, ShmRecvProxySetupInvalidReqSize) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));
  ...
  // Call the function with invalid reqSize
  ncclResult_t result = shmRecvProxySetup(&connection, &proxyState, &req, sizeof(req) - 1, &resp, sizeof(resp), &done);
...
TEST_F(ShmProxyTest, ShmRecvProxySetupInvalidRespSize) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));
  ...
  // Call the function with invalid respSize
  ncclResult_t result = shmRecvProxySetup(&connection, &proxyState, &req, sizeof(req), &resp, sizeof(resp) - 1, &done);
...
TEST_F(ShmProxyTest, ShmRecvProxySetupWithLegacy) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));
...
TEST_F(ShmProxyTest, ShmRecvProxySetup_FailPath) {
  // Setup variables
  struct ncclProxyConnection connection;
  memset(&connection, 0, sizeof(connection));
  ...
  // Call the function - should fail during ncclShmAllocateShareableBuffer
  ncclResult_t result = shmRecvProxySetup(
    &connection,
    &proxyState,
    &req, sizeof(req),
    &resp, sizeof(resp),
