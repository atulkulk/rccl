...
ncclResult_t ncclStrongStreamWaitStream(struct ncclCudaGraph cg, struct ncclStrongStream* stream_a, struct ncclStrongStream* stream_b, bool isDependent) {
  std::cout << "ncclStrongStreamWaitStream called" << std::endl;
  return g_mock->ncclStrongStreamWaitStream(cg, stream_a, stream_b, isDependent);
}

ncclResult_t ncclStrongStreamRelease(struct ncclCudaGraph cg, struct ncclStrongStream* ss) {
  std::cout << "ncclStrongStreamRelease called" << std::endl;
  return g_mock->ncclStrongStreamRelease(cg, ss);
}
...
TEST_F(IpcRegisterBufferTest, SuccessPath) {
  ...
  EXPECT_CALL(mock,
    ncclStrongStreamWaitStream(
        testing::A<struct ncclCudaGraph>(),
        testing::A<struct ncclStrongStream*>(),
        testing::A<struct ncclStrongStream*>(),
        testing::A<bool>()
    ))
    .WillOnce(testing::Return(ncclSuccess));
  EXPECT_CALL(mock, ncclStrongStreamRelease(testing::_, testing::_))
      .WillOnce([](struct ncclCudaGraph, struct ncclStrongStream*) {
        return ncclSuccess;
      });
