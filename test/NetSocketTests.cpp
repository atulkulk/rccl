...
TEST_F(NetSocketTests, TestInvalidDeviceIndexListen) {
  ...
  TEST_INFO("Listen with dev=-1 returned: %d", result);
  ...
  TEST_INFO("Listen with dev=%d (> ndev=%d) returned: %d", invalidDev,
       ndev, result);
  EXPECT_EQ(result, ncclInternalError)
...
TEST_F(NetSocketTests, TestInvalidDeviceIndexConnect) {
  ...
  TEST_INFO("Connect with dev=-1 returned: %d", result);
  EXPECT_EQ(result, ncclInternalError)
  ...
  result = ncclNetSocket.connect(invalidDev, &config, handle, &sendComm,
                                 &sendDevComm);
  TEST_INFO("Connect with dev=%d (> ndev=%d) returned: %d",
