...
// Forward declarations for functions defined in alt_rsmi.cc
namespace ARSMI {
namespace internal {
...
protected:
  ...
  void createFile(const std::string &path, const std::string &content) {
    ...
    if (!file) {
      std::cerr << "Failed to create file: " << path << errno << std::endl;
      return;
    }
    file << content;
    file.close();
  ...
  void SetUp() override {
    ...
    ARSMI::internal::kKFDNodesPathRoot = "/tmp/test_kfd/topology/nodes";

    // Create the test directory structure and files
    setupTestFiles();

    // Reset the thread-local variables before each test
    ARSMI::internal::ARSMI_orderedNodes.clear();
    ARSMI::internal::ARSMI_orderedLinks.clear();
    ARSMI::internal::ARSMI_num_devices = -1;
  }
...
TEST_F(AltRsmiTest, ARSMIGetNumDevicesUninitialized) {
  ARSMI::internal::ARSMI_num_devices = -1; // Force uninitialized state
  ...
  ASSERT_EQ(num_devices, ARSMI::internal::ARSMI_num_devices);
}
...
TEST_F(AltRsmiTest, ARSMIDevPciIdGetUninitialized) {
  ...
  ARSMI::internal::kKFDNodesPathRoot = "/invalid/path/to/file";
...
TEST_F(AltRsmiTest, GetNodeIndexInvalidNode) {
  ...
  int result = ARSMI::internal::getNodeIndex(invalid_node_id);
...
TEST_F(AltRsmiTest, DevicePathInvalidDeviceId) {
  ...
  std::string path = ARSMI::internal::DevicePath(invalid_dev_id);
...
TEST_F(AltRsmiTest, IsRegularFileInvalidPath) {
  ...
  bool is_reg = false;
  int result = ARSMI::internal::isRegularFile(invalid_path, &is_reg);
  ASSERT_NE(result, 0); // Expect non-zero error code
  ASSERT_FALSE(is_reg); // Expect is_reg to be false
}

TEST_F(AltRsmiTest, IsNumberInvalidInput) {
  ASSERT_FALSE(ARSMI::internal::isNumber("abc123")); // Non-numeric string
  ASSERT_FALSE(ARSMI::internal::isNumber(""));       // Empty string
  ASSERT_FALSE(ARSMI::internal::isNumber(" "));      // Whitespace string
}

TEST_F(AltRsmiTest, OpenNodeFileInvalidPath) {
  std::ifstream fs;
  int result = ARSMI::internal::openNodeFile(9999, "invalid_file", &fs);
  ASSERT_NE(result, 0);       // Expect non-zero error code
...
TEST_F(AltRsmiTest, OpenNodeFileNotRegularFile) {
  ...
  std::ifstream fs;
  int result = ARSMI::internal::openNodeFile(0, "properties", &fs);
...
TEST_F(AltRsmiTest, OpenNodeFileInvalidNodeFile) {
  ...
  std::ifstream fs;
  int result =
      ARSMI::internal::openNodeFile(invalid_dev_id, "invalid_file", &fs);
  ASSERT_NE(result, 0);       // Expect non-zero error code
  ASSERT_FALSE(fs.is_open()); // File stream should not be open
}
...
TEST_F(AltRsmiTest, CountIoLinksInvalidDeviceId) {
  uint32_t invalid_dev_id = 9999; // Device ID that doesn't exist
  int result = ARSMI::internal::countIoLinks(invalid_dev_id);
...
TEST_F(AltRsmiTest, OpenLinkFileInvalidLinkFile) {
  ...
  std::ifstream fs;
  int result = ARSMI::internal::openLinkFile(invalid_dev_id, invalid_target_id,
                                             "invalid_file", &fs);
  ASSERT_NE(result, 0);       // Expect non-zero error code
  ASSERT_FALSE(fs.is_open()); // File stream should not be open
}
...
TEST_F(AltRsmiTest, OpenLinkFileInvalidPath) {
  std::ifstream fs;
  int result = ARSMI::internal::openLinkFile(9999, 9999, "invalid_file", &fs);
  ASSERT_NE(result, 0);       // Expect non-zero error code
  ASSERT_FALSE(fs.is_open()); // File stream should not be open
}
...
TEST_F(AltRsmiTest, OpenLinkFileNotRegularFile) {
  ...
  std::ifstream fs;
  int result = ARSMI::internal::openLinkFile(0, 1, "properties", &fs);
...
TEST_F(AltRsmiTest, GetGpuIdInvalidNode) {
  uint64_t gpu_id = 0;
  int result = ARSMI::internal::getGpuId(9999, &gpu_id);
  ASSERT_NE(result, 0); // Expect non-zero error code
...
TEST_F(AltRsmiTest, ReadGpuIdInvalidNode) {
  uint32_t invalid_node_id = 9999; // Node ID that doesn't exist
  uint64_t gpu_id = 0;
  int result = ARSMI::internal::readGpuId(invalid_node_id, &gpu_id);
  ASSERT_NE(result, 0); // Expect non-zero error code
  ASSERT_EQ(gpu_id, 0); // GPU ID should not be modified
}
...
TEST_F(AltRsmiTest, ReadGpuIdInvalidData) {
  ...
  int result = ARSMI::internal::readGpuId(0, &gpu_id);
...
TEST_F(AltRsmiTest, IsNodeSupportedInvalidNode) {
  uint32_t invalid_node_id = 9999; // Node ID that doesn't exist
  bool result = ARSMI::internal::isNodeSupported(invalid_node_id);
...
TEST_F(AltRsmiTest, IsNodeSupportedEmptyFile) {
  ...
  bool result = ARSMI::internal::isNodeSupported(0);
...
TEST_F(AltRsmiTest, GetPropertyValueInvalidProperty) {
  ...
  uint64_t value = 0;
  int result =
      ARSMI::internal::getPropertyValue("invalid_property", &value, properties);
  ASSERT_NE(result, 0); // Expect non-zero error code
...
TEST_F(AltRsmiTest, GetPropertyValueNullValuePointer) {
  ...
  int result = ARSMI::internal::getPropertyValue("key1", value, properties);
...
TEST_F(AltRsmiTest, GetPropertyValueKeyNotFound) {
  ...
  int result = ARSMI::internal::getPropertyValue("key2", &value, properties);
...
TEST_F(AltRsmiTest, FileExistsInvalidPath) {
  ...
  bool result = ARSMI::internal::fileExists(invalid_path);
...
TEST_F(AltRsmiTest, ARSMIReadDevicePropertiesInvalidNode) {
  ...
  std::map<std::string, uint64_t> properties;
  int result =
      ARSMI::internal::ARSMI_readDeviceProperties(invalid_node_id, properties);
  ASSERT_NE(result, 0);            // Expect non-zero error code
...
TEST_F(AltRsmiTest, ARSMI_readDevicePropertiesNotRegularFile) {
  ...
  std::ifstream fs;
  int result = ARSMI::internal::ARSMI_readDeviceProperties(0, properties);
...
TEST_F(AltRsmiTest, ARSMIReadLinkPropertiesInvalidLink) {
  ...
  uint32_t invalid_target_id = 9999; // Target ID that doesn't exist
  std::map<std::string, uint64_t> properties;
  int result = ARSMI::internal::ARSMI_readLinkProperties(
...
TEST_F(AltRsmiTest, ARSMI_readLinkPropertiesNotRegularFile) {
  ...
  std::ifstream fs;
  int result = ARSMI::internal::ARSMI_readLinkProperties(0, 1, properties);
...
TEST_F(AltRsmiTest, ARSMI_readLinkPropertiesTrailingEmptyLine) {
  ...
  std::ifstream fs;
  int result = ARSMI::internal::ARSMI_readLinkProperties(0, 0, properties);
...
TEST_F(AltRsmiTest, ReadNodePropertiesInvalidProperty) {
  ...
  int result = ARSMI::internal::read_node_properties(0, "invalid_property",
                                                     &value, properties);
...
TEST_F(AltRsmiTest, ReadNodePropertiesInvalidNode) {
  ...
  std::map<std::string, uint64_t> properties;
  uint64_t value = 0;
  int result = ARSMI::internal::read_node_properties(
...
TEST_F(AltRsmiTest, ReadNodePropertiesInvalidPropertyValue) {
  ...
  std::map<std::string, uint64_t> properties;
  uint64_t *value = nullptr;
  int result = ARSMI::internal::read_node_properties(invalid_node_id, "", value,
                                                     properties);
...
TEST_F(AltRsmiTest, ReadLinkPropertiesInvalidLink) {
  ...
  std::map<std::string, uint64_t> properties;
  uint64_t value = 0;
  int result = ARSMI::internal::read_link_properties(
...
TEST_F(AltRsmiTest, SourceDeviceIndexOutOfRange) {
  ARSMI_linkInfo info;
  ARSMI::internal::ARSMI_num_devices =
...
TEST_F(AltRsmiTest, UninitializedNumDevices) {
  ...
  ARSMI::internal::kKFDNodesPathRoot =
      "/tmp/invalid_path"; // Simulate invalid path

  ARSMI_linkInfo info;
  ARSMI::internal::ARSMI_num_devices = -1; // Simulate uninitialized state
...
TEST_F(AltRsmiTest, InvalidLinkInfo) {
  // Simulate initialized state with two devices
  ARSMI::internal::ARSMI_num_devices = 2;
  ARSMI::internal::ARSMI_orderedNodes.push_back({0}); // Device 0
  ARSMI::internal::ARSMI_orderedNodes.push_back({1}); // Device 1

  // Initialize ARSMI_orderedLinks with data not in order
  ARSMI::internal::ARSMI_orderedLinks = {
      {
